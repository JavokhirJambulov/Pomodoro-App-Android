...
class MainScreenViewModel : ViewModel() {
    ...
    private fun checkTimer() {
        ...
        if (_currentTimer.value == TimerType.POMODORO) {
            ...
            if (_sessions.value == _sessionCounter.value) {
                ...
                startTimer(_longBreakTime.value?.times(60) ?: LONG_BREAK_DEFAULT_TIME)
            } else {
                _currentTimer.value = TimerType.BREAK
                startTimer(_breakTime.value?.times(60) ?: BREAK_DEFAULT_TIME)
            }
            ...
            startTimer(_pomodoroTime.value?.times(60) ?: BREAK_DEFAULT_TIME)
        } else {
    ...
    fun beginTimer() {
        _currentTimer.value = TimerType.POMODORO
        startTimer(_pomodoroTime.value?.times(60) ?: POMODOR_DEFAULT_TIME)
    }
